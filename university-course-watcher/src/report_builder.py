...
def build_graduate_admission_report(items: list[dict], path: Path = DATA_DIR / "graduate_admission_report.html") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    visible = sorted(
        items,
        key=lambda item: (GRADE_ORDER.get(item.get("grade"), 9), item.get("university_name", ""), item.get("title", "")),
    )
    rows = "\n".join(_graduate_admission_row(item) for item in visible) or "<tr><td colspan='12'>검색 결과가 없습니다.</td></tr>"
    content = f"""<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>2026 후기 일반대학원 모집요강 감시 리포트</title>
  <style>
    body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; margin: 24px; color: #1f2937; }}
    h1 {{ font-size: 24px; margin-bottom: 6px; }}
    .meta {{ color: #6b7280; margin-bottom: 18px; }}
...
<body>
  <h1>2026 후기 일반대학원 모집요강 감시 리포트</h1>
  <div class="meta">확인 일시: {html.escape(now_kst().isoformat(timespec="seconds"))}</div>
  <div class="notice">신규 텔레그램 알림은 A/B 등급의 새 모집요강 후보가 있을 때 전송됩니다. 신규 없음 요약은 결과 구성이 바뀐 경우에만 전송됩니다.</div>
  <div class="summary">
    <span>전체 후보 {len(visible)}건</span>
    <span>A등급 {sum(1 for item in visible if item.get("grade") == "A")}건</span>
    <span>B등급 {sum(1 for item in visible if item.get("grade") == "B")}건</span>
...
def _graduate_admission_row(item: dict) -> str:
    ...
    attachments = "<br>".join(f"<a href='{html.escape(url)}'>첨부</a>" for url in item.get("attachment_urls", []))
    return (
        f"<tr class='grade-{html.escape(item.get('grade', ''))}'>"
        f"<td>{'신규' if item.get('is_new') else '기존'}</td>"
        f"<td>{html.escape(item.get('grade', ''))}</td>"
